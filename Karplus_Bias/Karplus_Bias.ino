// floating point Karplus-Strong + bias distortion - pin PD0 audio PWM output //
...
float bias(float input){
  
  const float MIN_BIAS = 0.1f;
  const float MED_BIAS = 1.0f;
  const float MAX_BIAS = 6.0f;
  
  if (input < 0.5) {
    input = input * 2;
    return input*input * (MED_BIAS-MIN_BIAS) + MIN_BIAS;
  } else {
    input = (input - 0.5) * 2;
    return input*input * (MAX_BIAS-MED_BIAS) + MED_BIAS;
  }
