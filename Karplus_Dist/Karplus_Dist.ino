// floating point Karplus-Strong + distortion filter - pin PD0 audio PWM output //
...
float dist_filter(float input, float peak){

  input = input * 4.0f / 3.0f;
  return clip(input, peak);
...
void timerHandler(void) {
  ...
  if(trig) {
    ...
    out = out - (lowpass * (out - tmp));
	  ...
	  TIM1->CH1CVR = 128 + 127 * dist_filter(out, peak);
